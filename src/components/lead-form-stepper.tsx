...
export function LeadFormStepper() {
  ...
  const step1Schema = z.object({
    projectType: z.string().min(1, t('validation.project_type')),
  });

  const step2Schema = z.object({
    hasDesign: z.enum(['yes', 'no'], { required_error: t('validation.design_option') }),
    deadline: z.string().min(1, t('validation.deadline')),
    budget: z.string().min(1, t('validation.budget')),
  });
  ...
  const steps = [
    { id: '01', name: t('lead_form.steps.project_type'), fields: ['projectType'] },
    { id: '02', name: t('lead_form.steps.details'), fields: ['hasDesign', 'deadline', 'budget'] },
    { id: '03', name: t('lead_form.steps.contact'), fields: ['name', 'email', 'company', 'message'] },
  ];
  ...
  const onSubmit = async (data: FormData) => {
    ...
    toast({
      title: t('toast.form_submitted_title'),
      description: t('toast.form_submitted_description'),
    });
  };
  ...
  if (isSubmitted) {
    return (
        <div className="flex flex-col items-center justify-center text-center p-8 bg-secondary rounded-lg min-h-[500px]">
            ...
            <h2 className="font-headline text-2xl font-semibold text-foreground mb-2">{t('lead_form.submitted.title')}</h2>
            <p className="text-muted-foreground max-w-md">{t('lead_form.submitted.description')}</p>
        </div>
    );
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <RadioGroup {...field} onValueChange={field.onChange} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <h3 className="sm:col-span-2 font-semibold text-lg">{t('lead_form.step1.title')}</h3>
                        {[
                          { value: "corporate-website", label: t('lead_form.step1.options.corporate') },
                          { value: "ecommerce", label: t('lead_form.step1.options.ecommerce') },
                          { value: "custom-webapp", label: t('lead_form.step1.options.webapp') },
                          { value: "landing-page", label: t('lead_form.step1.options.landing') },
                          { value: "other", label: t('lead_form.step1.options.other') }
                        ].map(option => (
                ...
                >
                  <div className="space-y-8">
                    <Controller
                      ...
                      render={({ field }) => (
                        <RadioGroup {...field} onValueChange={field.onChange} className="space-y-2">
                          <Label className="text-lg">{t('lead_form.step2.design.label')}</Label>
                          <div className="flex gap-4">
                              <Label htmlFor="design-yes" className="flex items-center gap-2 border border-border rounded-md p-3 px-4 has-[:checked]:border-primary has-[:checked]:bg-primary/10">
                                  <RadioGroupItem value="yes" id="design-yes" /> {t('lead_form.step2.design.yes')}
                              </Label>
                              <Label htmlFor="design-no" className="flex items-center gap-2 border border-border rounded-md p-3 px-4 has-[:checked]:border-primary has-[:checked]:bg-primary/10">
                                  <RadioGroupItem value="no" id="design-no" /> {t('lead_form.step2.design.no')}
                              </Label>
                          </div>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <RadioGroup {...field} onValueChange={field.onChange} className="space-y-2">
                          <Label className="text-lg">{t('lead_form.step2.deadline.label')}</Label>
                          <div className="flex flex-wrap gap-4">
                            {[
                              { value: "~1 month", label: t('lead_form.step2.deadline.option1') },
                              { value: "2-3 months", label: t('lead_form.step2.deadline.option2') },
                              { value: "Flexible", label: t('lead_form.step2.deadline.option3') }
                            ].map(val => (
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                          <RadioGroup {...field} onValueChange={field.onChange} className="space-y-2">
                              <Label className="text-lg">{t('lead_form.step2.budget.label')}</Label>
                              <div className="flex flex-wrap gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Controller name="name" control={control} render={({ field }) => (
                          <div className="space-y-2">
                              <Label htmlFor="name">{t('lead_form.step3.name.label')}</Label>
                              <Input id="name" {...field} />
                      ...
                      <Controller name="email" control={control} render={({ field }) => (
                          <div className="space-y-2">
                              <Label htmlFor="email">{t('lead_form.step3.email.label')}</Label>
                      ...
                      <Controller name="company" control={control} render={({ field }) => (
                          <div className="space-y-2 sm:col-span-2">
                              <Label htmlFor="company">{t('lead_form.step3.company.label')}</Label>
                              <Input id="company" {...field} />
                          </div>
                      )}/>
                      <Controller name="message" control={control} render={({ field }) => (
                          <div className="space-y-2 sm:col-span-2">
                              <Label htmlFor="message">{t('lead_form.step3.message.label')}</Label>                              <Textarea id="message" rows={4} {...field} />
        ...
        <CardFooter className="flex justify-between">
          <Button type="button" variant="outline" onClick={prevStep} disabled={currentStep === 0}>
            {t('lead_form.buttons.previous')}
          </Button>
          {currentStep < steps.length - 1 ? (
            <Button type="button" onClick={nextStep}>
              {t('lead_form.buttons.next')} <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          ) : (
