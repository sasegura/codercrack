...
export default function Home() {
  ...
  const tLeadForm = useTranslations('LeadForm');
  ...
  return (
    <>
        ...
        <div className="container px-4 md:px-6 z-10 relative">
          ...
          <div className="mt-8 flex justify-center gap-4 flex-wrap">
            <Button size="lg" asChild>
              <Link href="#projects">
                {t('hero_cta_projects')} <ArrowRight className="ml-2 h-5 w-5" />
              </Link>
            </Button>
            <Button size="lg" variant="outline" asChild>
              <Link href="#lead-form">{t('hero_cta_consultation')}</Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section id="servicios" className="py-20 md:py-28 bg-secondary">
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">{tServices('title')}</h2>
            ...
            {services.map((service) => (
              <Card key={service.id} className="flex flex-col text-center items-center bg-background/50 hover:bg-background/80 transition-all duration-300 hover:-translate-y-1 border-primary/10">
                <CardHeader className="items-center">
                  ...
                  <CardTitle className="font-headline text-xl">{tServices(`${service.id}.title`)}</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-muted-foreground">{tServices(`${service.id}.description`)}</p>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section id="lead-form" className="py-20 md:py-28 bg-background">
        <div className="container px-4 md:px-6">
          <div className="max-w-3xl mx-auto text-center mb-12">
            <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">{tLeadForm('title')}</h2>
            <p className="max-w-2xl mx-auto mt-4 text-muted-foreground">
              {tLeadForm('subtitle')}
            </p>
          </div>
      ...
      <section id="proyectos" className="py-20 md:py-28 bg-background">
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">{tProjects('title')}</h2>
            <p className="max-w-2xl mx-auto mt-4 text-muted-foreground">
              {tProjects('subtitle')}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project) => {
              ...
              <Card key={project.id} className="overflow-hidden group bg-secondary/50 border-primary/10">
                {image && <div className="overflow-hidden"><Image src={image.imageUrl} alt={tProjects(project.titleKey)} width={600} height={400} data-ai-hint={image.imageHint} className="w-full h-auto object-cover transition-transform duration-300 group-hover:scale-105"/></div>}
                <CardContent className="p-6">
                  <h3 className="text-xl font-headline font-semibold mb-2">{tProjects(project.titleKey)}</h3>
                  <p className="text-muted-foreground mb-4 text-sm">{tProjects(project.descriptionKey)}</p>
      ...
      <section id="testimonios" className="py-20 md:py-28 bg-secondary">
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">{tTestimonials('title')}</h2>
          </div>
                ...
                <CarouselItem key={index}>
                  <Card className="bg-background">
                    <CardContent className="p-8 text-center">
                      <p className="text-lg italic mb-6 text-foreground/80">"{tTestimonials(testimonial.quoteKey)}"</p>
        ...
        <div className="container px-4 md:px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="relative aspect-square max-w-md mx-auto">
              {aboutMeImage && <Image src={aboutMeImage.imageUrl} alt={tAbout('image_alt')} data-ai-hint={aboutMeImage.imageHint} fill className="rounded-lg object-cover shadow-lg shadow-primary/10"/>}
            </div>
            <div>
              <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground mb-4">{tAbout('title')}</h2>
              <p className="text-muted-foreground mb-6 text-lg">
                {tAbout('paragraph')}
              </p>
              <div className="space-y-4">
                <div className="flex items-start gap-3"><CheckCircle className="h-6 w-6 text-primary mt-1 shrink-0" /><p><span className="font-semibold">{tAbout('feature1_title')}</span> {tAbout('feature1_text')}</p></div>
                <div className="flex items-start gap-3"><CheckCircle className="h-6 w-6 text-primary mt-1 shrink-0" /><p><span className="font-semibold">{tAbout('feature2_title')}</span> {tAbout('feature2_text')}</p></div>
                <div className="flex items-start gap-3"><CheckCircle className="h-6 w-6 text-primary mt-1 shrink-0" /><p><span className="font-semibold">{tAbout('feature3_title')}</span> {tAbout('feature3_text')}</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="contacto" className="py-20 md:py-28 bg-secondary">
        <div className="container px-4 md:px-6">
          <div className="max-w-2xl mx-auto text-center">
              <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">{tContact('title')}</h2>
